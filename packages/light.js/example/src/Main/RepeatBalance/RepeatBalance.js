...
class RepeatBalance extends Component {
  render() {
    const { balanceOf, chainName, blockNumber, me } = this.props;
    return (
      <div className="RepeatBalance-container">
        <h3>This is RepeatBalance component.</h3>
        <p>
          It shows some same info as Balance component.
          <br />
          It's to show that even though the Observables have more subscribers,
          the underlying api calls are not duplicated, unless necessary.
          <br />
        </p>
        <p>
          Chain: {chainName}. Block: {+blockNumber}. My Account: {me}.
        </p>
        <p>
          ...
          .<br />
          (notice the number of <code>eth_getBalance</code> calls)
        </p>
      </div>
    );
  }
